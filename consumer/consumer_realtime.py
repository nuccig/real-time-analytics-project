...
SNS_TOPIC_ARN = "arn:aws:sns:us-east-1:551715065713:snsalerta"

# Ler valores das variáveis de ambiente
PRECIPITATION_PROBABILITY_THRESHOLD = int(
    os.environ.get("PRECIPITATION_PROBABILITY", 10)
)
...
def lambda_handler(event, context):
    ...
    for record in event["Records"]:
        ...
        if (
            ...
            response = sns_client.publish(
                TopicArn=SNS_TOPIC_ARN, Message=message, Subject="Alerta Meteorológico"
            )
    ...
    return {
        "statusCode": 200,
        "body": json.dumps(
            "Processed Kinesis records and sent to SNS if thresholds exceeded"
        ),
    }
